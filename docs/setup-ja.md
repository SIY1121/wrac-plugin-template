...
### VST3 / AU、または開発用 standalone app をビルドする場合

clap-wrapper を用いて VST3 / AU を生成する場合、または開発用 standalone app をビルドする場合は、追加で以下が必要です。
...
プラグイン identity は plugin package の manifest、初期状態では `plugins/wrac-gain/src-plugin/Cargo.toml` に集約しています。
...
リリースビルドでは `src-plugin/build.rs` が隣接する `src-gui/dist` を zip 化して plugin バイナリに埋め込むため、dev server は不要です。
...
Standalone app は軽量な開発用 host であり、リリース用のプラグインフォーマットや出荷 artifact ではありません。
