...
validator は test ID ごとに DSH `runtest` を実行し、公式 JSON result を
`target/wrac-plugins/<package>/wrac/validation/aax/` に保存します。選択された test の
`result_status` が pass 以外なら `xtask` は失敗します。
