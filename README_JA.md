...
`cargo xtask build` は現在の OS で対応している target をすべてビルドします。
macOS では CLAP/VST3/AU/standalone、Windows では CLAP/VST3/standalone、
Linux では CLAP のみです。
`build --target` に `clap`、`vst3`、`au`、`standalone` のカンマ区切りリストを
渡すと、必要なものだけビルドできます。`install` / `uninstall` の target は
plugin format の `clap`、`vst3`、`au` のみです。

成果物は `target/wrac/artifacts/<profile>/` に、スタンドアローンアプリは
`target/wrac/standalone/<profile>/` に配置されます。macOS では、`--validate` が
VST3 validator と `auval -v aufx WtGn YrCo` を実行します。
